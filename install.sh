...
install_x-ui() {
    ...
    # TUIC runs as a separate sidecar binary (internal/tuic). Upstream ships it
    # only inside its release tarball, so the fork's own archive is the source;
    # without it every TUIC inbound fails with
    # "fork/exec bin/tuic-server: no such file or directory". The panel checks
    # bin/tuic-server-<os>-<goarch> first, then the plain name.
    if [[ -f bin/tuic-server ]]; then
        cp -f bin/tuic-server "bin/tuic-server-linux-$(arch)"
        chmod +x bin/tuic-server "bin/tuic-server-linux-$(arch)"
    else
        echo -e "${yellow}[WARN] bin/tuic-server is missing from the release -- TUIC inbounds will not start until it is placed in bin/ manually.${plain}"
    fi
