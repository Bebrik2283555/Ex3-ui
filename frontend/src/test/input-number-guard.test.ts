...
function runGuard(target: string): string {
  // Invoke the JS entry via node directly: the .bin shims are POSIX-only and
  // Windows blocks .cmd spawning without a shell.
  try {
    execFileSync(process.execPath, ['node_modules/oxlint/bin/oxlint', '-c', `${FIXTURES}/guard.oxlintrc.json`, target], {
      encoding: 'utf8',
      stdio: 'pipe',
    });
    return '';
