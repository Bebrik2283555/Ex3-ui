...
function ServiceCard({ name, displayName, isDark, isUltra }: { name: CoreName; displayName: string; isDark: boolean; isUltra: boolean }) {
  ...
  const coreTitle = (
    <div className="extras-card-title">
  ...
  return (
    <>
      <Card
        ...
        extra={headerActions}
        variant="borderless"
        className={`${cardClass} extras-core-card`}
        actions={actions}
      >
              ...
              <div className="extras-connect">
                ...
                <div className="extras-link">
                  ...
                  <Text type="secondary" style={{ display: 'block' }}>
                    ...
                    {t('pages.extras.olcrtcTransport')}: {service.config?.transport} ·{' '}
                    {t('pages.extras.olcrtcDns')}: {service.config?.olcrtcDns}
                  </Text>
                </div>
              </div>
            </div>
...
export default function ExtrasPage() {
  ...
  const [activeCore, setActiveCore] = useState<CoreName>('qwdtt');
  const current = cores.find((c) => c.name === activeCore) ?? cores[0];
