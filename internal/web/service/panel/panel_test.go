...
func TestUpdateProxyEnvVars(t *testing.T) {
	...
	t.Run("forwards each set var under its own name", func(t *testing.T) {
		...
		want := []string{
			"https_proxy=socks5://127.0.0.1:10808", "HTTPS_PROXY=socks5://127.0.0.1:10808",
			"no_proxy=10.0.0.0/8,localhost", "NO_PROXY=10.0.0.0/8,localhost",
		}
	...
	t.Run("http_proxy is not promoted to https_proxy", func(t *testing.T) {
		...
		got := updateProxyEnvVars()
		want := []string{"http_proxy=http://127.0.0.1:8080", "HTTP_PROXY=http://127.0.0.1:8080"}
