...
func TestResolveRequest_ForwardedHeaderTrust(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "no stored row ignores forwarded headers from outside localhost",
			stored:           nil,
		...
		{
			name:             "empty stored value ignores forwarded headers",
			stored:           new(""),
			remoteAddr:       "203.0.113.9:51000",
			wantScheme:       "http",
			wantHost:         "panel.example.com",
		...
		{
			name:             "stored shipped default trusts only localhost",
		...
		{
			name:             "shipped default trusts a localhost proxy",
			stored:           new(service.DefaultTrustedProxyCIDRs),
